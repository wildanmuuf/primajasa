...
if(isset($_POST['simpan'])){
	...
	$id_kostumer = $_SESSION['id_petugas'];
	$tgl_berangkat = $_POST['tgl_berangkat'];
	$total = $_POST['total'];
	echo $id_kostumer;
	$tgl = date("Y-m-d");
	$jam = date("H:i");
	$sql="INSERT INTO orders (id_orders, tgl_berangkat, tgl_order, jam_order, id_petugas, total) VALUES ('$id_orders', '$tgl_berangkat', '$tgl', '$jam','$id_kostumer', '$total')";
	...
	if($simpan){
		...
		while($rCheck = mysqli_fetch_array($check)){
			$sql_detail = "INSERT INTO detail_kursi (id, id_orders, id_tiket, no_kursi) VALUES ('','$id_orders','$rCheck[id_tiket_temp]','$rCheck[no_kursi_temp]')";
